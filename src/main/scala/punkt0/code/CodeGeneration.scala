...
object CodeGeneration extends Phase[Program, Unit] {
  ...
  def run(prog: Program)(ctx: Context): Unit = {
    ...
    def generateClassFile(sourceName: String, ct: ClassDecl, dir: String): Unit = {
      ...
      val parentName: Option[String] = if (ct.parent.isDefined) Some(ct.parent.get.value) else Some("java/lang/Object")
      ...
      ct.methods.foreach { method: MethodDecl =>
        val ch: CodeHandler = classFile.addMethod(toByteCodeType(method.getType), method.id.value, method.args.map { arg: Formal =>
    ...
    def generateMethodCode(ch: CodeHandler, mt: MethodDecl): Unit = {
      ...
      val optimizedMethodExpr: List[ExprTree] = optimizeExprs(mt.exprs, mt.retType.getType)
      (optimizedMethodExpr.map(compileExpr(_, ch)) ::: List(compileExpr(mt.retExpr, ch)))
    ...
    def toByteCodeType(tpe: Type): String = tpe match {
      ...
      case _         => Reporter.fatal("This type should not be here....")
    }
    ...
    def toByteCodeTypes(tpe: TypeTree): String = tpe match {
      ...
      case _             => Reporter.fatal("This type tree should not be here....")
    }
          ...
          expr match {
            ...
            case Println(expr: ExprTree) =>
              ...
              expr.getType match {
                ...
                case _ => sys.error("Println cannot print this type, wrong code gen");
              }
            ...
            case MethodCall(obj: ExprTree, meth: Identifier, args: List[ExprTree]) =>
              ...
              val methodSig = "(" + ms.argTpeList.foldLeft("")((acc, arg) => acc +
                toByteCodeType(arg)) + ")" + toByteCodeType(ms.tpe)
    ...
    def generateMainMethodCode(ch: CodeHandler, stmts: List[ExprTree], cname: String): Unit = {
