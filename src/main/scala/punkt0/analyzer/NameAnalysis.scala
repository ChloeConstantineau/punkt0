...
object NameAnalysis extends Phase[Program, Program] {
  ...
  val acceptableAssignType = List(New.toString(), Null.toString(), IntLit.toString(), StringLit.toString(),
  ...
  def run(prog: Program)(ctx: Context): Program = {
      ...
      case Identifier(value) =>
        { //Check Class exists
          if (!globalScope.classes.contains(value))
            Reporter.fatal("This type (" + value + ") is not declared, it cannot be used", tpe)
    ...
    def makeVariableSymbol(aTree: Tree): VariableSymbol = aTree match {
      case VarDecl(tpe, id, expr) => new VariableSymbol(id.value, getType(tpe)).setPos(aTree)
      case Formal(tpe, id)        => new VariableSymbol(id.value, getType(tpe)).setPos(aTree)
      case _                      => Reporter.fatal("Expected a varriable declaration or a formal declaration", aTree)
    }

    def makeMethodSymbol(method: MethodDecl, classSymbol: ClassSymbol): MethodSymbol = {

      verifyOverrideConstraints(method, classSymbol)

      val methodType = getType(method.retType)
      val newMethodSymbol = new MethodSymbol(method.id.value, classSymbol, methodType).setPos(method)

      newMethodSymbol.params = attachMethodParams(method.args)
      newMethodSymbol.members = attachMembers(method.vars)

      if (method.overrides) {
        var overridenMethodSymbol = classSymbol.lookupMethod(method.id.toString())
    ...
    def verifyOverrideConstraints(method: MethodDecl, classSymbol: ClassSymbol): Unit = {
      ...
      if (method.overrides) {
        ...
        if (!parent.isDefined)
          Reporter.error("This class has no parent, there can be no overriding of methods", method)
        ...
        if (!matchingMethod.isDefined)
          Reporter.error("The parent tree does not contains such a method, overriding does not apply", method)

        //Both methods have the same number of parameters
        if (matchingMethod.get.params.size != method.args.size)
          Reporter.error("This method does not have the same number of parameters as the method it overrides", method)

        //Return type must match
        if (!matchingMethod.get.tpe.equals(getType(method.retType)))
          Reporter.error("The return type of the overridden fonction does not match the original, it needs to be of type: " + matchingMethod.get.tpe, method)
        ...
        for ((param, i) <- matchingMethod.get.params.zipWithIndex) {
          if (!param._2.tpe.equals(getType(method.args(i).tpe)))
            Reporter.error("The param in the overriding method does not match the original signature of the method, the parameter " + method.args(i).id + " has the wrong type", method.args(i))
        }
      }

      //No method has same name in class and his parents unless the class method has 'override'
      if (!method.overrides && classSymbol.lookupMethodInParentTree(method.id.value).isDefined) {
        Reporter.error("This method has already been declared in the parent tree. Solution is to override the method", method)
      }
    }
    ...
    def attachClasses(): Unit = {
      ...
      for (i <- 0 until prog.classes.length) {
        ...
        var newClassSymbol = new ClassSymbol(prog.classes(i).id.value).setPos(prog.classes(i))
    ...
    def checkConstraintClassDecl(classDecl: ClassDecl): Unit = {
      ...
      if (classDecl.id.value.equals("Main"))
        Reporter.error("Main was already declared at line" + globalScope.mainClass.posString, classDecl)

      //Class has already been declared
      if (globalScope.lookupClass(classDecl.id.value) != None)
        Reporter.error("A class with this name has already been declared at line " + globalScope.lookupClass(classDecl.id.value).get.posString, classDecl)
    }
    ...
    def attachParentClasses(): Unit = {
      ...
      for (i <- 0 until prog.classes.length) {
        ...
        if (prog.classes(i).parent.isDefined) {
          ...
          classSymbol.parent = globalScope.classes.get(prog.classes(i).parent.get.value)
        }
      }
    ...
    def checkParentConstraintClassDecl(classDecl: ClassDecl, newClassSymbol: ClassSymbol): Unit = {
      //Check Parent Class Exists
      if (!globalScope.classes.contains(classDecl.parent.get.value))
        Reporter.error("The parent class is not declared: " + classDecl.parent.get.value, classDecl.parent.get)
    }
      ...
      if (classSymbol.parent.get.parent.isDefined) {
        ...
        while (nextGeneration.isDefined) {
          if (visitedClasses.contains(nextGeneration.get))
            Reporter.fatal("The class declared has a cycle of dependendies with its parents", classSymbol)
          else {
    ...
    def attachMain(): Unit = {
      ...
      globalScope.mainClass.parent = Some(new ClassSymbol(prog.main.parent.value))
    ...
    def analyseVarAssign(vars: List[VarDecl], classSymbol: ClassSymbol, methodSymbol: Option[MethodSymbol]): Unit = {

      for (i <- 0 until vars.length if vars.length.>(0)) {

        //Check type variable is of Id type and if this class exists
        if (vars(i).tpe.getClass.getSimpleName.equals(Identifier.toString()) && !globalScope.classes.contains(vars(i).tpe.toString.slice(11, vars(i).tpe.toString.length() - 1)))
          Reporter.fatal("This type (" + vars(i).tpe.toString() + ") is not declared, it cannot be used", vars(i))
    ...
    def checkAssignIsValid(variable: ExprTree): Unit = {
      if (!acceptableAssignType.contains(variable.getClass.getSimpleName)) {
        Reporter.error("This variable assign is not permitted : " + variable.getClass.getSimpleName + ". Must be of type : New, Null, Boolean, IntLit or StringLit.", variable)
      }
    }
    ...
    def attachMembers(vars: List[VarDecl]): Map[String, VariableSymbol] = {
      ...
      for (i <- 0 until vars.length if vars.length.>(0)) {
        if (!mapMembers.contains(vars(i).id.value)) {
          ...
          Reporter.error("This variable has already been declared at line " + mapMembers.get(vars(i).id.value).get.posString, vars(i))
      }

      mapMembers.toMap
    }

    def attachMethods(classSymbol: ClassSymbol, methods: List[MethodDecl]): Map[String, MethodSymbol] = {
      ...
      for (i <- 0 until methods.length if (methods.length.>(0))) {
        if (!mapMethods.contains(methods(i).id.value)) {
          ...
          Reporter.error("This method has already been declared here " + mapMethods.get(methods(i).id.value).get.posString, methods(i))
      }

      mapMethods.toMap
    }

    def attachMethodParams(params: List[Formal]): Map[String, VariableSymbol] = {
    ...
    def getExpr(expr: ExprTree, classSymbol: ClassSymbol, methodSymbol: Option[MethodSymbol]): Unit = {
      expr match {
    ...
    def getExprInternal(expr: ExprTree, classSymbol: ClassSymbol, methodSymbol: Option[MethodSymbol]): Unit = {
      expr match {
        case Identifier(id) => {

          //Is a method member or param
          if (methodSymbol.isDefined && methodSymbol.get.lookupVar(id).isDefined) {
            expr.asInstanceOf[Identifier].setSymbol(methodSymbol.get.lookupVar(id).get)

            //Is a class member
          } else if (classSymbol.lookupVar(id).isDefined) {
            expr.asInstanceOf[Identifier].setSymbol(classSymbol.lookupVar(id).get)
            ...
            val newMemberSymbol = new VariableSymbol(inheritedMember.name, inheritedMember.tpe)

            expr.asInstanceOf[Identifier].setSymbol(newMemberSymbol)
            globalScope.classes.get(classSymbol.name).get.members += newMemberSymbol.name -> newMemberSymbol

          } else {
            Reporter.error("This variable has never been declared or is of type not definied")
          }
        }
        ...
        expr match {
          case Identifier(id) => {
            if (globalScope.lookupClass(id).isDefined) {
              expr.asInstanceOf[Identifier].setSymbol(globalScope.lookupClass(id).get)
            } else
              Reporter.error("Cannot create an object of class " + id + ". This class has not been defined", expr)
          }
          case _ => Reporter.fatal("Expected an Identifier..")
        }
      }
    }
