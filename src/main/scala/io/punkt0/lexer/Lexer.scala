    ...
    def parseLines(
        ...
        def parseLine(
            ...
            line match
                case ::(head, tail) =>
                  head match
                      ...
                      case '&' | '|' | '=' =>
                        ...
                        if nextTokenKind == EQSIGN || nextTokenKind == BAD then
                            parseLine(tail, tokens :+ token, x + 1)
                        else parseLine(tail.drop(1), tokens :+ token, x + 2)
                      case alphaR(_*)      =>
                        val matched =
                      ...
                      case numericalR(_*)  =>
                        ...
                        val token   =
                          if number.startsWith("0") && number.length > 1 then Token(BAD, cursor)
                ...
                case Nil            =>
                  if lines.isEmpty then tokens :+ Token(EOF, cursor)
                  else tokens
